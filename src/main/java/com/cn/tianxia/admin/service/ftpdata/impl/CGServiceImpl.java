 ...
 * @Description TODO(CG视讯注单查询服务)
 * @author Hardy
 * @Date 2019年5月15日 下午5:24:39
 * @version 1.0.0
 */
@Service
public class CGServiceImpl implements CGService {
    ...
    @Autowired
    private GameRecordCommonMapper<CgRecord> commonMapper;
    @Autowired
    ...
    @Override
    public Pager<CgRecord> list(Pager<CgRecord> pager) throws Exception {
        int queryType = Integer.parseInt(pager.getParams().get("queryType"));
        Pager<CgRecord> resultPager = null;
        // 如果查询条件是 按照用户真实姓名查询
    ...
    @Override
    public Pager<CgRecord> listRecordByRealName(Pager<CgRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<CgRecord> agBrList = cgRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
    ...
    @Override
    public Pager<CgRecord> listRecordBytopUidName(Pager<CgRecord> pager) {
    ...
    @Override
    public Pager<CgRecord> listRecordByUserTypeName(Pager<CgRecord> pager) {
    ...
    @Override
    public Pager<CgRecord> listRecordByAgUserName(Pager<CgRecord> pager) {
        // 查询注单结果
        List<CgRecord> agBrList = cgRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
    ...
    @Override
    public Pager<CgRecord> handleRecordList(List<CgRecord> agBrList, Pager<CgRecord> pager) {
        ...
        if(!"0".equals(userGrouping)){
            ...
            List<Map<String, Object>> countResult = cgRecordMapper.countTotal(pager);
            //开始汇总
...
//        pager.getParams().put("playTypeTmp", playTypeTmp);
        ...
        agBrList = agBrList.parallelStream().flatMap(agbr -> usersList.parallelStream()
                .filter(tUser -> agbr.getUsername().equals(tUser.getAgUsername())).map(tUser -> {
                    ...
                    .filter(gameTypeMap -> agbr.getGametype().equals(gameTypeMap.get("gameType"))).map(gameTypeMap -> {
                        ...
                        // CG视讯特殊处理 gameType = "BACCARAT_INSURANCE" 在 platform_dic_data不存在， BACCARAT_INSURANCE以 BACCARAT处理
                        ...
                        .map(betType -> dicDataList.parallelStream()
                                ...
                                .map(dicData -> {
                                        ...
                                        // CG 游戏记录特殊处理，游戏记录根据注单里的betType后几位 + platform_dic_data 
                                        //中的dataValue 拼接而成
        ...
        List<Map<String, Object>> countResult = cgRecordMapper.countTotal(pager);
        //开始汇总
        Pager<CgRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<CgRecord> doCountSum(List<Map<String, Object>> countResult, List<CgRecord> agBrList,
            Pager<CgRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<CgRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        if("0".equals(userGrouping)){
            ...
            pageValidTotalNum = agBrList.parallelStream().mapToInt(map -> map.getSequenceno()).sum();
        }
        ...
        Pager<CgRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
