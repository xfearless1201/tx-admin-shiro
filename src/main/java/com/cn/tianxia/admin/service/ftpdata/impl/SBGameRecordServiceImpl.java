...
import com.cn.tianxia.admin.domain.ftpdata.SBGameRecordMapper;
...
import com.cn.tianxia.admin.service.ftpdata.SBGameRecordService;
import com.cn.tianxia.admin.service.txdata.SysUserService;

/**
 * 
 * @ClassName SBGameRecordServiceImpl
 * @Description TODO(申博电子注单查询服务)
 * @author Hardy
 * @Date 2019年5月16日 下午10:03:57
 * @version 1.0.0
 */
@Service
public class SBGameRecordServiceImpl implements SBGameRecordService {
    
    private Logger logger = LoggerFactory.getLogger(SBGameRecordServiceImpl.class);
    
    @Autowired
    private SBGameRecordMapper sbGameRecordMapper;
    @Autowired
    private GameRecordCommonMapper<SBGameRecord> commonMapper;
    @Autowired
    private SysUserService sysUserService;

    @SuppressWarnings("unchecked")
    @Override
    public Pager<SBGameRecord> transFormDataToPager(FormData data) {
        try {
    ...
    @Override
    public Pager<SBGameRecord> list(Pager<SBGameRecord> pager) throws Exception {
        ...
        if("1".equals(timeType)){
            ...
            timeType = "betupdatedon";
        }
        pager.getParams().put("timeType",timeType);
        
        Pager<SBGameRecord> resultPager = null;
    ...
    @Override
    public Pager<SBGameRecord> listRecordByRealName(Pager<SBGameRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<SBGameRecord> agBrList = sbGameRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
    ...
    @Override
    public Pager<SBGameRecord> listRecordBytopUidName(Pager<SBGameRecord> pager) {
    ...
    @Override
    public Pager<SBGameRecord> listRecordByUserTypeName(Pager<SBGameRecord> pager) {
    ...
    @Override
    public Pager<SBGameRecord> listRecordByAgUserName(Pager<SBGameRecord> pager) {
        // 查询注单结果
        List<SBGameRecord> agBrList = sbGameRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
    ...
    @Override
    public Pager<SBGameRecord> handleRecordList(List<SBGameRecord> agBrList, Pager<SBGameRecord> pager) {
        ...
        if(!"0".equals(userGrouping)){
            ...
            List<Map<String, Object>> countResult = sbGameRecordMapper.countTotal(pager);
            //开始汇总
        ...
        List<Map<String, Object>> countResult = sbGameRecordMapper.countTotal(pager);
        //开始汇总
        Pager<SBGameRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<SBGameRecord> doCountSum(List<Map<String, Object>> countResult, List<SBGameRecord> agBrList,
            Pager<SBGameRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<SBGameRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        Pager<SBGameRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
