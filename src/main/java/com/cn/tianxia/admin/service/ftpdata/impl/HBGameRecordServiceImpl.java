...
import com.cn.tianxia.admin.domain.ftpdata.HBGameRecordMapper;
...
import com.cn.tianxia.admin.service.ftpdata.HBGameRecordService;
import com.cn.tianxia.admin.service.txdata.SysUserService;

/**
 * 
 * @ClassName HBGameRecordServiceImpl
 * @Description TODO(HABA电子注单查询服务)
 * @author Hardy
 * @Date 2019年5月17日 下午6:26:53
 * @version 1.0.0
 */
@Service
public class HBGameRecordServiceImpl implements HBGameRecordService{

    private Logger logger = LoggerFactory.getLogger(HBGameRecordServiceImpl.class);
    
    @Autowired
    private HBGameRecordMapper hbGameRecordMapper;
    @Autowired
    private GameRecordCommonMapper<HBGameRecord> commonMapper;
    @Autowired
    ...
    @Override
    public Pager<HBGameRecord> transFormDataToPager(FormData data) {
        try {
    ...
    @Override
    public Pager<HBGameRecord> list(Pager<HBGameRecord> pager) throws Exception {
        
        int queryType = Integer.parseInt(pager.getParams().get("queryType"));
        Pager<HBGameRecord> resultPager = null;
        // 如果查询条件是 按照用户真实姓名查询
    ...
    @Override
    public Pager<HBGameRecord> listRecordByRealName(Pager<HBGameRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<HBGameRecord> agBrList = hbGameRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
    ...
    @Override
    public Pager<HBGameRecord> listRecordBytopUidName(Pager<HBGameRecord> pager) {
    ...
    @Override
    public Pager<HBGameRecord> listRecordByUserTypeName(Pager<HBGameRecord> pager) {
    ...
    @Override
    public Pager<HBGameRecord> listRecordByAgUserName(Pager<HBGameRecord> pager) {
        // 查询注单结果
        List<HBGameRecord> agBrList = hbGameRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
    ...
    @Override
    public Pager<HBGameRecord> handleRecordList(List<HBGameRecord> agBrList, Pager<HBGameRecord> pager) {
        ...
        if(!"0".equals(userGrouping)){
            ...
            List<Map<String, Object>> countResult = hbGameRecordMapper.countTotal(pager);
            //开始汇总
        ...
        // 根据游戏gameKey查询游戏类型
        String gameTypeTmp = agBrList.parallelStream().map(agbr -> agbr.getGamekeyname()).collect(Collectors.toSet()).parallelStream()
        ...
        agBrList = agBrList.parallelStream().flatMap(agbr -> usersList.parallelStream()
                .filter(tUser -> agbr.getUsername().equals(tUser.getAgUsername())).map(tUser -> {
                    ...
                    .filter(gameTypeMap -> agbr.getGamekeyname().equals(gameTypeMap.get("gameType"))).map(gameTypeMap -> {
        ...
        List<Map<String, Object>> countResult = hbGameRecordMapper.countTotal(pager);
        //开始汇总
        Pager<HBGameRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<HBGameRecord> doCountSum(List<Map<String, Object>> countResult, List<HBGameRecord> agBrList,
            Pager<HBGameRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<HBGameRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        BigDecimal pageNetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(map.getPayout().toString())).sum());
        ...
        Pager<HBGameRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
