...
import com.cn.tianxia.admin.domain.ftpdata.ObRecordMapper;
...
import com.cn.tianxia.admin.service.ftpdata.OBService;
import com.cn.tianxia.admin.service.txdata.SysUserService;

/**
 * 
 * @ClassName OBServiceIMpl
 * @Description TODO(欧博视讯注单查询服务)
 * @author Hardy
 * @Date 2019年5月14日 下午2:47:55
 * @version 1.0.0
 */
@Service
public class OBServiceImpl implements OBService {
    ...
    @Autowired
    private ObRecordMapper obRecordMapper;
    @Autowired
    private GameRecordCommonMapper<ObRecord> commonMapper;
    @Autowired
    private SysUserService sysUserService;
    Logger logger = LoggerFactory.getLogger(OBServiceImpl.class);
    
    @SuppressWarnings("unchecked")
    @Override
    public Pager<ObRecord> transFormDataToPager(FormData data) {
        try {
    ...
    @Override
    public Pager<ObRecord> list(Pager<ObRecord> pager) throws Exception {
        int queryType = Integer.parseInt(pager.getParams().get("queryType"));
        Pager<ObRecord> resultPager = null;
        // 如果查询条件是 按照用户真实姓名查询
    ...
    @Override
    public Pager<ObRecord> listRecordByRealName(Pager<ObRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<ObRecord> agBrList = obRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
        ...
        //从查询结果中拿出查到的会员账号
        userName = agBrList.parallelStream().map(agbr -> agbr.getClient()).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        pager.getParams().put("userName", userName);
    ...
    @Override
    public Pager<ObRecord> listRecordBytopUidName(Pager<ObRecord> pager) {
    ...
    @Override
    public Pager<ObRecord> listRecordByUserTypeName(Pager<ObRecord> pager) {
    ...
    @Override
    public Pager<ObRecord> listRecordByAgUserName(Pager<ObRecord> pager) {
        // 查询注单结果
        List<ObRecord> agBrList = obRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
        ...
        // 根据注单结果中的会员账号查询会员信息
        String userName = agBrList.parallelStream().map(agbr -> agbr.getClient()).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        pager.getParams().put("userName", userName);
    ...
    @Override
    public Pager<ObRecord> handleRecordList(List<ObRecord> agBrList, Pager<ObRecord> pager) {
        ...
        if(!"0".equals(userGrouping)){
            ...
            List<Map<String, Object>> countResult = obRecordMapper.countTotal(pager);
            //开始汇总
        ...
        String playTypeTmp = agBrList.parallelStream().map(agbr -> String.valueOf(agbr.getBettype())).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        ...
        agBrList = agBrList.parallelStream().flatMap(agbr -> usersList.parallelStream()
                .filter(tUser -> agbr.getClient().equals(tUser.getAgUsername())).map(tUser -> {
                    ...
                    .filter(gameTypeMap -> agbr.getGametype().equals(gameTypeMap.get("gameType"))).map(gameTypeMap -> {
                     ...
                     .filter(dicData -> dicData.get("dataKey").equals(agbr.getBettype().toString()) 
        ...
        List<Map<String, Object>> countResult = obRecordMapper.countTotal(pager);
        //开始汇总
        Pager<ObRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<ObRecord> doCountSum(List<Map<String, Object>> countResult, List<ObRecord> agBrList,
            Pager<ObRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<ObRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        BigDecimal pageNetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(map.getWinorloss().toString())).sum());
        ...
        if("0".equals(userGrouping)){
            ...
            pageValidTotalNum = agBrList.parallelStream().mapToInt(map -> map.getCommission()).sum();
        }
        ...
        Pager<ObRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
