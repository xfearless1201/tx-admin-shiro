...
import com.cn.tianxia.admin.domain.ftpdata.JDBGameRecordMapper;
...
import com.cn.tianxia.admin.service.ftpdata.JDBGameRecordService;
import com.cn.tianxia.admin.service.txdata.SysUserService;

/**
 * 
 * @ClassName JDBGameRecordServiceImpl
 * @Description TODO(JDB电子注单查询服务)
 * @author Hardy
 * @Date 2019年5月18日 下午4:11:56
 * @version 1.0.0
 */
@Service
public class JDBGameRecordServiceImpl implements JDBGameRecordService{

    private Logger logger = LoggerFactory.getLogger(JDBGameRecordServiceImpl.class);
    
    @Autowired
    private JDBGameRecordMapper jdbGameRecordMapper;
    @Autowired
    private GameRecordCommonMapper<JDBGameRecord> commonMapper;
    @Autowired
    private SysUserService sysUserService;
    
    @SuppressWarnings("unchecked")
    @Override
    public Pager<JDBGameRecord> transFormDataToPager(FormData data) {
        try {
    ...
    @Override
    public Pager<JDBGameRecord> list(Pager<JDBGameRecord> pager) throws Exception {
        
        int queryType = Integer.parseInt(pager.getParams().get("queryType"));
        Pager<JDBGameRecord> resultPager = null;
        // 如果查询条件是 按照用户真实姓名查询
    ...
    @Override
    public Pager<JDBGameRecord> listRecordByRealName(Pager<JDBGameRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<JDBGameRecord> agBrList = jdbGameRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
        ...
        //从查询结果中拿出查到的会员账号
        userName = agBrList.parallelStream().map(agbr -> agbr.getPlayerid()).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        pager.getParams().put("userName", userName);
    ...
    @Override
    public Pager<JDBGameRecord> listRecordBytopUidName(Pager<JDBGameRecord> pager) {
    ...
    @Override
    public Pager<JDBGameRecord> listRecordByUserTypeName(Pager<JDBGameRecord> pager) {
    ...
    @Override
    public Pager<JDBGameRecord> listRecordByAgUserName(Pager<JDBGameRecord> pager) {
        // 查询注单结果
        List<JDBGameRecord> agBrList = jdbGameRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
        ...
        // 根据注单结果中的会员账号查询会员信息
        String userName = agBrList.parallelStream().map(agbr -> agbr.getPlayerid()).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        pager.getParams().put("userName", userName);
    ...
    @Override
    public Pager<JDBGameRecord> handleRecordList(List<JDBGameRecord> agBrList, Pager<JDBGameRecord> pager) {
        ...
        if(!"0".equals(userGrouping)){
            ...
            List<Map<String, Object>> countResult = jdbGameRecordMapper.countTotal(pager);
            //开始汇总
        ...
        // 根据游戏gameKey查询游戏类型
        String gameTypeTmp = agBrList.parallelStream().map(agbr -> agbr.getMtype().toString()
        ...
        agBrList = agBrList.parallelStream().flatMap(agbr -> usersList.parallelStream()
                .filter(tUser -> agbr.getPlayerid().equals(tUser.getAgUsername())).map(tUser -> {
                    agbr.setRealName(tUser.getRealname());
                    return agbr;
                })
            ).flatMap(agbr -> gameTypeList.parallelStream()
                    .filter(gameTypeMap -> agbr.getMtype().toString().equals(gameTypeMap.get("gameType"))).map(gameTypeMap -> {
        ...
        List<Map<String, Object>> countResult = jdbGameRecordMapper.countTotal(pager);
        //开始汇总
        Pager<JDBGameRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<JDBGameRecord> doCountSum(List<Map<String, Object>> countResult, List<JDBGameRecord> agBrList,
            Pager<JDBGameRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<JDBGameRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        BigDecimal pageBetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(
                String.valueOf(Math.abs(map.getBet())))).sum());
        BigDecimal pageNetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(map.getTotal().toString())).sum());
        BigDecimal pageValidBetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(
                String.valueOf(Math.abs(map.getValidbet())))).sum());
        // 如果不是按照用户分组
        if("0".equals(userGrouping)){
        ...
        Pager<JDBGameRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
