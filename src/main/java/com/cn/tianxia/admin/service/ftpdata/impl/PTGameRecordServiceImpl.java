...
import com.cn.tianxia.admin.domain.ftpdata.PTGameRecordMapper;
...
import com.cn.tianxia.admin.service.ftpdata.PTGameRecordService;
import com.cn.tianxia.admin.service.txdata.SysUserService;
/**
 * 
 * @ClassName PTGameRecordServiceImpl
 * @Description TODO(PT电子注单查询服务)
 * @author Hardy
 * @Date 2019年5月17日 下午8:13:54
 * @version 1.0.0
 */
@Service
public class PTGameRecordServiceImpl implements PTGameRecordService {
    
    private Logger logger = LoggerFactory.getLogger(PTGameRecordServiceImpl.class);
    
    @Autowired
    private PTGameRecordMapper ptGameRecordMapper;
    @Autowired
    private GameRecordCommonMapper<PTGameRecord> commonMapper;
    @Autowired
    private SysUserService sysUserService;

    @SuppressWarnings("unchecked")
    @Override
    public Pager<PTGameRecord> transFormDataToPager(FormData data) {
        try {
    ...
    @Override
    public Pager<PTGameRecord> list(Pager<PTGameRecord> pager) throws Exception {
        
        int queryType = Integer.parseInt(pager.getParams().get("queryType"));
        Pager<PTGameRecord> resultPager = null;
        // 如果查询条件是 按照用户真实姓名查询
    ...
    @Override
    public Pager<PTGameRecord> listRecordByRealName(Pager<PTGameRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<PTGameRecord> agBrList = ptGameRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
    ...
    @Override
    public Pager<PTGameRecord> listRecordBytopUidName(Pager<PTGameRecord> pager) {
    ...
    @Override
    public Pager<PTGameRecord> listRecordByUserTypeName(Pager<PTGameRecord> pager) {
    ...
    @Override
    public Pager<PTGameRecord> listRecordByAgUserName(Pager<PTGameRecord> pager) {
        // 查询注单结果
        List<PTGameRecord> agBrList = ptGameRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
    ...
    @Override
    public Pager<PTGameRecord> handleRecordList(List<PTGameRecord> agBrList, Pager<PTGameRecord> pager) {
        ...
        if(!"0".equals(userGrouping)){
            ...
            List<Map<String, Object>> countResult = ptGameRecordMapper.countTotal(pager);
            //开始汇总
        ...
        String gameTypeTmp = agBrList.parallelStream().map(agbr -> {
                    String gameName = agbr.getGamename();
                    gameName = gameName.substring(0, gameName.indexOf("("));
                    agbr.setGamename(gameName);
                    return gameName;
        ...
        List<Map<String, Object>> countResult = ptGameRecordMapper.countTotal(pager);
        //开始汇总
        Pager<PTGameRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<PTGameRecord> doCountSum(List<Map<String, Object>> countResult, List<PTGameRecord> agBrList,
            Pager<PTGameRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<PTGameRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        Pager<PTGameRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
