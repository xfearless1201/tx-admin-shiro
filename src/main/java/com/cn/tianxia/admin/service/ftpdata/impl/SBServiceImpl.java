...
import com.cn.tianxia.admin.domain.ftpdata.SbRecordMapper;
...
import com.cn.tianxia.admin.service.ftpdata.SBService;
import com.cn.tianxia.admin.service.txdata.SysUserService;

/**
 * 
 * @ClassName SBServiceImpl
 * @Description TODO(申博视讯注单查询服务)
 * @author Hardy
 * @Date 2019年5月14日 下午10:53:24
 * @version 1.0.0
 */
@Service
public class SBServiceImpl implements SBService {
    ...
    @Autowired
    private GameRecordCommonMapper<SbRecord> commonMapper;
    @Autowired
    private SysUserService sysUserService;
    Logger logger = LoggerFactory.getLogger(SBServiceImpl.class);
    
    @SuppressWarnings("unchecked")
    @Override
    public Pager<SbRecord> transFormDataToPager(FormData data) {
        try {
    ...
    @Override
    public Pager<SbRecord> list(Pager<SbRecord> pager) throws Exception {
        int queryType = Integer.parseInt(pager.getParams().get("queryType"));
        Pager<SbRecord> resultPager = null;
        // 如果查询条件是 按照用户真实姓名查询
    ...
    @Override
    public Pager<SbRecord> listRecordByRealName(Pager<SbRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<SbRecord> agBrList = sbRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
    ...
    @Override
    public Pager<SbRecord> listRecordBytopUidName(Pager<SbRecord> pager) {
    ...
    @Override
    public Pager<SbRecord> listRecordByUserTypeName(Pager<SbRecord> pager) {
    ...
    @Override
    public Pager<SbRecord> listRecordByAgUserName(Pager<SbRecord> pager) {
        // 查询注单结果
        List<SbRecord> agBrList = sbRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
    ...
    @Override
    public Pager<SbRecord> handleRecordList(List<SbRecord> agBrList, Pager<SbRecord> pager) {
        ...
        if(!"0".equals(userGrouping)){
            ...
            List<Map<String, Object>> countResult = sbRecordMapper.countTotal(pager);
            //开始汇总
        ...
        // 根据注单结果中的游戏类型查询ag视讯的游戏类型,如果类型是BACCARAT_INSURANCE 则按照BACCARAT的类型处理
        // 先以gameType 查询出platform_game中CG配置
        String gameTypeTmp = agBrList.parallelStream().map(agbr -> agbr.getGamename()         
        ...
        // 根据游戏类型结果 查询数据字典表
        // 申博视讯特殊处理,申博视讯中platform_dic_data表里data_type字段与shenbo_record表里的playtype字段一致,playtype该字段决定data_value
        String dataTypeTmp = agBrList.parallelStream().map(agbr -> agbr.getPlaytype()).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        ...
        agBrList = agBrList.parallelStream().flatMap(agbr -> usersList.parallelStream()
                .filter(tUser -> agbr.getUsername().equals(tUser.getAgUsername())).map(tUser -> {
                    ...
                    .filter(gameTypeMap -> agbr.getGamename().equals(gameTypeMap.get("gameType"))).map(gameTypeMap -> {
                        agbr.setGameTypeName(gameTypeMap.get("gameTypeName"));
                        return agbr;
                    })
             ).flatMap(agbr -> dicDataList.parallelStream()
                    .filter(dicData -> agbr.getPlaytype().equalsIgnoreCase(dicData.get("dataType"))).map(dicData -> {
        ...
        List<Map<String, Object>> countResult = sbRecordMapper.countTotal(pager);
        //开始汇总
        Pager<SbRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<SbRecord> doCountSum(List<Map<String, Object>> countResult, List<SbRecord> agBrList,
            Pager<SbRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<SbRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        BigDecimal pageBetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(
                String.valueOf(Math.abs(map.getRiskamt())))).sum());
        ...
        BigDecimal pageValidBetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(
                String.valueOf(Math.abs(map.getRollingturnover())))).sum());
        // 如果不是按照用户分组
        if("0".equals(userGrouping)){
            pageTotalNum = agBrList.size();
            pageValidTotalNum = agBrList.parallelStream().mapToInt(map -> map.getRollingturnover() > 0 ? 1 : 0 ).sum();
        }else{
            pageTotalNum = agBrList.parallelStream().mapToInt(map -> map.getId()).sum();
            pageValidTotalNum = agBrList.parallelStream().mapToInt(map -> Integer.valueOf(map.getIpaddress())).sum();
        }
        ...
        Pager<SbRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
