...
import com.cn.tianxia.admin.domain.ftpdata.PSGameRecordMapper;
...
import com.cn.tianxia.admin.service.ftpdata.PSGameRecordService;
import com.cn.tianxia.admin.service.txdata.SysUserService;

/**
 * 
 * @ClassName PSGameRecordServiceImpl
 * @Description TODO(PS电子注单查询服务)
 * @author Hardy
 * @Date 2019年5月18日 上午11:14:40
 * @version 1.0.0
 */
@Service
public class PSGameRecordServiceImpl implements PSGameRecordService {
    
    private Logger logger = LoggerFactory.getLogger(PSGameRecordServiceImpl.class);
    
    @Autowired
    private PSGameRecordMapper psGameRecordMapper;
    @Autowired
    private GameRecordCommonMapper<PSGameRecord> commonMapper;
    @Autowired
    private SysUserService sysUserService;

    @SuppressWarnings("unchecked")
    @Override
    public Pager<PSGameRecord> transFormDataToPager(FormData data) {
        try {
    ...
    @Override
    public Pager<PSGameRecord> list(Pager<PSGameRecord> pager) throws Exception {
        
        int queryType = Integer.parseInt(pager.getParams().get("queryType"));
        Pager<PSGameRecord> resultPager = null;
        // 如果查询条件是 按照用户真实姓名查询
    ...
    @Override
    public Pager<PSGameRecord> listRecordByRealName(Pager<PSGameRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<PSGameRecord> agBrList = psGameRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
    ...
    @Override
    public Pager<PSGameRecord> listRecordBytopUidName(Pager<PSGameRecord> pager) {
    ...
    @Override
    public Pager<PSGameRecord> listRecordByUserTypeName(Pager<PSGameRecord> pager) {
    ...
    @Override
    public Pager<PSGameRecord> listRecordByAgUserName(Pager<PSGameRecord> pager) {
        // 查询注单结果
        List<PSGameRecord> agBrList = psGameRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
    ...
    @Override
    public Pager<PSGameRecord> handleRecordList(List<PSGameRecord> agBrList, Pager<PSGameRecord> pager) {
        ...
        if(!"0".equals(userGrouping)){
            ...
            List<Map<String, Object>> countResult = psGameRecordMapper.countTotal(pager);
            //开始汇总
        ...
        // 根据游戏gameKey查询游戏类型
        String gameTypeTmp = agBrList.parallelStream().map(agbr -> agbr.getGid()
        ...
        agBrList = agBrList.parallelStream().flatMap(agbr -> usersList.parallelStream()
                .filter(tUser -> agbr.getUsername().equals(tUser.getAgUsername())).map(tUser -> {
                    ...
                    .filter(gameTypeMap -> agbr.getGid().equals(gameTypeMap.get("gameType"))).map(gameTypeMap -> {
        ...
        List<Map<String, Object>> countResult = psGameRecordMapper.countTotal(pager);
        //开始汇总
        Pager<PSGameRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<PSGameRecord> doCountSum(List<Map<String, Object>> countResult, List<PSGameRecord> agBrList,
            Pager<PSGameRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<PSGameRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        Pager<PSGameRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
