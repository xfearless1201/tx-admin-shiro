...
import com.cn.tianxia.admin.domain.ftpdata.SWGameRecordMapper;
...
import com.cn.tianxia.admin.service.ftpdata.SWGameRecordService;
import com.cn.tianxia.admin.service.txdata.SysUserService;
/**
 * 
 * @ClassName SWGameRecordServiceImpl
 * @Description TODO(SW电子注单查询服务)
 * @author Hardy
 * @Date 2019年5月18日 下午12:04:14
 * @version 1.0.0
 */
@Service
public class SWGameRecordServiceImpl implements SWGameRecordService {
    
    private Logger logger = LoggerFactory.getLogger(SWGameRecordServiceImpl.class);
    
    @Autowired
    private SWGameRecordMapper swGameRecordMapper;
    @Autowired
    private GameRecordCommonMapper<SWGameRecord> commonMapper;
    @Autowired
    private SysUserService sysUserService;

    @SuppressWarnings("unchecked")
    @Override
    public Pager<SWGameRecord> transFormDataToPager(FormData data) {
        try {
    ...
    @Override
    public Pager<SWGameRecord> list(Pager<SWGameRecord> pager) throws Exception {
        
        int queryType = Integer.parseInt(pager.getParams().get("queryType"));
        Pager<SWGameRecord> resultPager = null;
        // 如果查询条件是 按照用户真实姓名查询
    ...
    @Override
    public Pager<SWGameRecord> listRecordByRealName(Pager<SWGameRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<SWGameRecord> agBrList = swGameRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
        ...
        //从查询结果中拿出查到的会员账号
        userName = agBrList.parallelStream().map(agbr -> agbr.getPlayercode()).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        pager.getParams().put("userName", userName);
    ...
    @Override
    public Pager<SWGameRecord> listRecordBytopUidName(Pager<SWGameRecord> pager) {
    ...
    @Override
    public Pager<SWGameRecord> listRecordByUserTypeName(Pager<SWGameRecord> pager) {
    ...
    @Override
    public Pager<SWGameRecord> listRecordByAgUserName(Pager<SWGameRecord> pager) {
        // 查询注单结果
        List<SWGameRecord> agBrList = swGameRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
        ...
        // 根据注单结果中的会员账号查询会员信息
        String userName = agBrList.parallelStream().map(agbr -> agbr.getPlayercode()).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        pager.getParams().put("userName", userName);
    ...
    @Override
    public Pager<SWGameRecord> handleRecordList(List<SWGameRecord> agBrList, Pager<SWGameRecord> pager) {
        ...
        if(!"0".equals(userGrouping)){
            ...
            List<Map<String, Object>> countResult = swGameRecordMapper.countTotal(pager);
            //开始汇总
        ...
        agBrList = agBrList.parallelStream().flatMap(agbr -> usersList.parallelStream()
                .filter(tUser -> agbr.getPlayercode().equals(tUser.getAgUsername())).map(tUser -> {
        ...
        List<Map<String, Object>> countResult = swGameRecordMapper.countTotal(pager);
        //开始汇总
        Pager<SWGameRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<SWGameRecord> doCountSum(List<Map<String, Object>> countResult, List<SWGameRecord> agBrList,
            Pager<SWGameRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<SWGameRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        BigDecimal pageNetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(map.getRevenue().toString())).sum());
        ...
        Pager<SWGameRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
