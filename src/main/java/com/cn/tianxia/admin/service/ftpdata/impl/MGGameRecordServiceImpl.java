...
import com.cn.tianxia.admin.domain.ftpdata.MGGameRecordMapper;
...
import com.cn.tianxia.admin.service.ftpdata.MGGameRecordService;
import com.cn.tianxia.admin.service.txdata.SysUserService;
/**
 * 
 * @ClassName MGGameRecordServiceImpl
 * @Description TODO(MG电子注单查询服务)
 * @author Hardy
 * @Date 2019年5月17日 下午5:37:02
 * @version 1.0.0
 */
@Service
public class MGGameRecordServiceImpl implements MGGameRecordService{
    
    private Logger logger = LoggerFactory.getLogger(MGGameRecordServiceImpl.class);
    
    @Autowired
    private MGGameRecordMapper mgGameRecordMapper;
    @Autowired
    private GameRecordCommonMapper<MGGameRecord> commonMapper;
    @Autowired
    private SysUserService sysUserService;

    @SuppressWarnings("unchecked")
    @Override
    public Pager<MGGameRecord> transFormDataToPager(FormData data) {
        try {
    ...
    @Override
    public Pager<MGGameRecord> list(Pager<MGGameRecord> pager) throws Exception {
        
        int queryType = Integer.parseInt(pager.getParams().get("queryType"));
        Pager<MGGameRecord> resultPager = null;
        // 如果查询条件是 按照用户真实姓名查询
    ...
    @Override
    public Pager<MGGameRecord> listRecordByRealName(Pager<MGGameRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<MGGameRecord> agBrList = mgGameRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
        ...
        //从查询结果中拿出查到的会员账号
        userName = agBrList.parallelStream().map(agbr -> agbr.getMbrusername()).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        pager.getParams().put("userName", userName);
    ...
    @Override
    public Pager<MGGameRecord> listRecordBytopUidName(Pager<MGGameRecord> pager) {
    ...
    @Override
    public Pager<MGGameRecord> listRecordByUserTypeName(Pager<MGGameRecord> pager) {
    ...
    @Override
    public Pager<MGGameRecord> listRecordByAgUserName(Pager<MGGameRecord> pager) {
        // 查询注单结果
        List<MGGameRecord> agBrList = mgGameRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
        ...
        // 根据注单结果中的会员账号查询会员信息
        String userName = agBrList.parallelStream().map(agbr -> agbr.getMbrusername()).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        pager.getParams().put("userName", userName);
    ...
    @Override
    public Pager<MGGameRecord> handleRecordList(List<MGGameRecord> agBrList, Pager<MGGameRecord> pager) {
        ...
        if(!"0".equals(userGrouping)){
            ...
            List<Map<String, Object>> countResult = mgGameRecordMapper.countTotal(pager);
            //开始汇总
        ...
        // 根据游戏gameKey查询游戏类型
        String gameTypeTmp = agBrList.parallelStream().map(agbr -> {
                String gameKey = agbr.getGamekey();
                gameKey = gameKey.substring(gameKey.indexOf(":")+1);
                agbr.setGamekey(gameKey);
                return gameKey;
        ...
        agBrList = agBrList.parallelStream().flatMap(agbr -> usersList.parallelStream()
                .filter(tUser -> agbr.getMbrusername().equals(tUser.getAgUsername())).map(tUser -> {
                    agbr.setRealName(tUser.getRealname());
                    return agbr;
                })
            ).flatMap(agbr -> gameTypeList.parallelStream()
                    .filter(gameTypeMap -> agbr.getGamekey().equals(gameTypeMap.get("gameType"))).map(gameTypeMap -> {
        ...
        List<Map<String, Object>> countResult = mgGameRecordMapper.countTotal(pager);
        //开始汇总
        Pager<MGGameRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<MGGameRecord> doCountSum(List<Map<String, Object>> countResult, List<MGGameRecord> agBrList,
            Pager<MGGameRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<MGGameRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        BigDecimal pageBetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(
                String.valueOf(Math.abs(map.getAmount())))).sum());
        BigDecimal pageNetAmount ;
        BigDecimal pageValidBetAmount ;
...
//        sum(if(type = 'mgsapiwin', 0,1)) as gameCasinoId
        ...
        if("0".equals(userGrouping)){
            ...
            pageValidTotalNum = agBrList.parallelStream().mapToInt(map -> "mgsapiwin".equals(map.getType()) ? 0 : 1).sum();
            pageBetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(
                    String.valueOf("mgsaspibet".equals(map.getType()) ? map.getAmount() : 0))).sum());
            pageNetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> {
                        String type = map.getType();
                        if("mgsapiwin".equals(type)){
                            return map.getAmount();
                        }else{
                            if("mgsapirefund".equals(type)){
                                return map.getAmount();
                            }else{
                                return -Math.abs(map.getAmount());
                            }
                        }
                    }).sum());
            pageValidBetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> {
                    String type = map.getType();
            ...
            pageValidTotalNum = agBrList.parallelStream().mapToInt(map -> Integer.parseInt(map.getGamecasinoid())).sum();
            pageBetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(
                    String.valueOf(Math.abs(map.getAmount())))).sum());
            pageNetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(map.getMbrcode().toString())).sum());
            pageValidBetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(
                    String.valueOf(map.getMbralias()))).sum());
        }
        ...
        Pager<MGGameRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
