...
import com.cn.tianxia.admin.domain.ftpdata.CQJGameRecordMapper;
...
import com.cn.tianxia.admin.service.ftpdata.CQJGameRecordService;
import com.cn.tianxia.admin.service.txdata.SysUserService;

/**
 * 
 * @ClassName CQJGameRecordServiceImpl
 * @Description TODO(CQJ电子注单查询服务)
 * @author Hardy
 * @Date 2019年5月18日 下午5:26:05
 * @version 1.0.0
 */
@Service
public class CQJGameRecordServiceImpl implements CQJGameRecordService {

    private Logger logger = LoggerFactory.getLogger(CQJGameRecordServiceImpl.class);
    
    @Autowired
    private CQJGameRecordMapper cqjGameRecordMapper;
    @Autowired
    private GameRecordCommonMapper<CQJGameRecord> commonMapper;
    @Autowired
    private SysUserService sysUserService;
    
    @SuppressWarnings("unchecked")
    @Override
    public Pager<CQJGameRecord> transFormDataToPager(FormData data) {
        try {
    ...
    @Override
    public Pager<CQJGameRecord> list(Pager<CQJGameRecord> pager) throws Exception {
        
        int queryType = Integer.parseInt(pager.getParams().get("queryType"));
        Pager<CQJGameRecord> resultPager = null;
        // 如果查询条件是 按照用户真实姓名查询
    ...
    @Override
    public Pager<CQJGameRecord> listRecordByRealName(Pager<CQJGameRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<CQJGameRecord> agBrList = cqjGameRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
        ...
        //从查询结果中拿出查到的会员账号
        userName = agBrList.parallelStream().map(agbr -> agbr.getAccount()).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        pager.getParams().put("userName", userName);
    ...
    @Override
    public Pager<CQJGameRecord> listRecordBytopUidName(Pager<CQJGameRecord> pager) {
    ...
    @Override
    public Pager<CQJGameRecord> listRecordByUserTypeName(Pager<CQJGameRecord> pager) {
    ...
    @Override
    public Pager<CQJGameRecord> listRecordByAgUserName(Pager<CQJGameRecord> pager) {
        // 查询注单结果
        List<CQJGameRecord> agBrList = cqjGameRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
        ...
        // 根据注单结果中的会员账号查询会员信息
        String userName = agBrList.parallelStream().map(agbr -> agbr.getAccount()).collect(Collectors.toSet()
                ).parallelStream().collect(Collectors.joining("','", "('", "')"));
        pager.getParams().put("userName", userName);
    ...
    @Override
    public Pager<CQJGameRecord> handleRecordList(List<CQJGameRecord> agBrList, Pager<CQJGameRecord> pager) {
        ...
        if(!"0".equals(userGrouping)){
            ...
            List<Map<String, Object>> countResult = cqjGameRecordMapper.countTotal(pager);
            //开始汇总
        ...
        // 根据游戏gameKey查询游戏类型
        String gameTypeTmp = agBrList.parallelStream().map(agbr -> agbr.getGamecode()
        ...
        agBrList = agBrList.parallelStream().flatMap(agbr -> usersList.parallelStream()
                .filter(tUser -> agbr.getAccount().equals(tUser.getAgUsername())).map(tUser -> {
                    agbr.setRealName(tUser.getRealname());
                    return agbr;
                })
            ).flatMap(agbr -> gameTypeList.parallelStream()
                    .filter(gameTypeMap -> agbr.getGamecode().equals(gameTypeMap.get("gameType"))).map(gameTypeMap -> {
        ...
        List<Map<String, Object>> countResult = cqjGameRecordMapper.countTotal(pager);
        //开始汇总
        Pager<CQJGameRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<CQJGameRecord> doCountSum(List<Map<String, Object>> countResult, List<CQJGameRecord> agBrList,
            Pager<CQJGameRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<CQJGameRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        BigDecimal pageNetAmount = new BigDecimal(agBrList.parallelStream().mapToDouble(map -> Double.parseDouble(map.getWin().toString())).sum());
        ...
        Pager<CQJGameRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
