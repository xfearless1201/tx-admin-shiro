...
import com.cn.tianxia.admin.domain.ftpdata.YoPlayGameRecordMapper;
...
import com.cn.tianxia.admin.service.ftpdata.YoplayGameRecordSerevice;
import com.cn.tianxia.admin.service.txdata.SysUserService;

/**
 * 
 * @ClassName YoplayGameRecordSereviceImpl
 * @Description TODO(Yoplay电子注单查询服务)
 * @author Hardy
 * @Date 2019年5月17日 下午10:51:14
 * @version 1.0.0
 */
@Service
public class YoplayGameRecordSereviceImpl implements YoplayGameRecordSerevice {

    private Logger logger = LoggerFactory.getLogger(YoplayGameRecordSereviceImpl.class);
    
    @Autowired
    private YoPlayGameRecordMapper yoPlayGameRecordMapper;
    @Autowired
    private GameRecordCommonMapper<YoPlayGameRecord> commonMapper;
    @Autowired
    ...
    @Override
    public Pager<YoPlayGameRecord> transFormDataToPager(FormData data) {
        try {
    ...
    @Override
    public Pager<YoPlayGameRecord> list(Pager<YoPlayGameRecord> pager) throws Exception {
        int queryType = Integer.parseInt(pager.getParams().get("queryType"));
        Pager<YoPlayGameRecord> resultPager = null;
        // 如果查询条件是 按照用户真实姓名查询
    ...
    @Override
    public Pager<YoPlayGameRecord> listRecordByRealName(Pager<YoPlayGameRecord> pager) {
        // 先通过realName 查询出用户信息
        ...
        //根据查询到的会员账号反查出游戏记录
        List<YoPlayGameRecord> agBrList = yoPlayGameRecordMapper.listGameRecord(pager);
        // 没有查询到则返回
        if(agBrList.size() ==0){
    ...
    @Override
    public Pager<YoPlayGameRecord> listRecordBytopUidName(Pager<YoPlayGameRecord> pager) {
    ...
    @Override
    public Pager<YoPlayGameRecord> listRecordByUserTypeName(Pager<YoPlayGameRecord> pager) {
    ...
    @Override
    public Pager<YoPlayGameRecord> listRecordByAgUserName(Pager<YoPlayGameRecord> pager) {
        // 查询注单结果
        List<YoPlayGameRecord> agBrList = yoPlayGameRecordMapper.listGameRecord(pager);
        if(agBrList.size() == 0){
            return doCountSum(null, agBrList, pager);
    ...
    @Override
    public Pager<YoPlayGameRecord> handleRecordList(List<YoPlayGameRecord> agBrList, Pager<YoPlayGameRecord> pager) {
      //如果是分组查询
        String userGrouping = pager.getParams().get("userGrouping");
        if(!"0".equals(userGrouping)){
            //统计总数 总条数，总投注额，有效输赢额
            GameRecordQuery.setContitions(pager);
            List<Map<String, Object>> countResult = yoPlayGameRecordMapper.countTotal(pager);
            //开始汇总
        ...
        List<Map<String, Object>> countResult = yoPlayGameRecordMapper.countTotal(pager);
        //开始汇总
        Pager<YoPlayGameRecord> resultPager = doCountSum(countResult,agBrList,pager);
        return resultPager;
    }

    @Override
    public Pager<YoPlayGameRecord> doCountSum(List<Map<String, Object>> countResult, List<YoPlayGameRecord> agBrList,
            Pager<YoPlayGameRecord> pager) {
        ...
        if(agBrList.size() == 0){
            ...
            Pager<YoPlayGameRecord> resultPager = new Pager<>(0, pager.getPage(), pager.getLimit());
        ...
        if("0".equals(userGrouping)){
            ...
            pageValidTotalNum = agBrList.parallelStream().mapToInt(agbr -> agbr.getValidbetamount() > 0 ? 1 : 0).sum();
        }else{
        ...
        Pager<YoPlayGameRecord> resultPager = new Pager<>(totalNum, pager.getPage(), pager.getLimit());
        resultPager.setList(agBrList);
        resultPager.setParams(map1);
